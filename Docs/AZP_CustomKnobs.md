...
| `bAZP_HideBodySkin` | `bool` | `true` | Camera\|Body | EditAnywhere + BP | Master toggle: hides every FP body section named in AZP_HiddenBodySections at BeginPlay (2026-08-05: skin/under-body geometry poked through the pants on look-down). |
| `AZP_HiddenBodySections` | `TArray<FName>` | `MI_Skin, Fbx_Default_Material_1` | Camera\|Body | EditAnywhere + BP | Material slot names whose sections get hidden — MI_Skin = the authored under-outfit skin; Fbx_Default_Material_1 = FBX-import leftover under-body geometry wearing the pants material. Add/remove names if anything still pokes through or goes missing (applied at BeginPlay). |
| `bAZP_ApparelSkinMask` | `bool` | `true` | Camera\|Body | EditAnywhere + BP | Pack-native apparel skin masking (2026-08-06: skin through the pants on look-down, pants mesh + legs must stay). M_Skin_CCMH WPO-shrinks body verts painted in ApparelMask_0/1/2 so covered skin tucks INSIDE the clothing — the customizer's own dress-up step our manual SetupMarcusAppearance skipped. Dynamic MI on MarcusBody only; shared skin MI asset untouched. |
| `AZP_ApparelMaskUpper` | `TSoftObjectPtr<UTexture>` | `T_ApparelMask_FullBody` | Camera\|Body | EditAnywhere + BP | ApparelMask_0 (Upper Body slot). Overalls_01's own DT row value. Empty = neutral T_DefaultWPO_Mask (no shrink). |
| `AZP_ApparelMaskLower` | `TSoftObjectPtr<UTexture>` | *(empty)* | Camera\|Body | EditAnywhere + BP | ApparelMask_1 (Lower Body slot). Marcus has no lower-only garment — neutral. |
| `AZP_ApparelMaskFootwear` | `TSoftObjectPtr<UTexture>` | `T_ApparelMask_Shoes` | Camera\|Body | EditAnywhere + BP | ApparelMask_2 (Footwear slot). Sneaker_02's own DT row value. |
| `AZP_ApparelNeckShrink` | `float` | `1.0f` | Camera\|Body | EditAnywhere + BP | 'Neck Shrink' scalar on the body skin MID (Overalls_01 row sets 1 — tucks the body's neck stub under the collar). Negative = leave MI default. |
| `AZP_ApparelMaskMultUpper` | `float` | `5.0f` | Camera\|Body | EditAnywhere + BP | 'Upper Body_Mask_Mult' shrink strength for ApparelMask_0 (MI default 3; raised for the pipe-walk right-leg poke). Negative = MI default. |
| `AZP_ApparelMaskMultLower` | `float` | `-1.0f` | Camera\|Body | EditAnywhere + BP | 'Lower Body_Mask_Mult' shrink strength for ApparelMask_1. Negative = MI default (3). |
| `AZP_ApparelMaskMultFootwear` | `float` | `-1.0f` | Camera\|Body | EditAnywhere + BP | 'Footwear_Mask_Mult' shrink strength for ApparelMask_2. Negative = MI default (1). |
| `AZP_UnarmedGloveOverlay` | `TSoftObjectPtr<UTexture>` | `T_Marcus_GloveOverlay` | Camera\|Body | EditAnywhere + BP | Unarmed glove match: hand-UV-only overlay via the skin material's Tattoo layer (base-color lerp; transparent off-hand). Painted in the KagiVision M_Gloves average tone so unarmed hands match the weapon view models. Empty = bare skin. |
| `bAZP_CameraInteractLOS` | `bool` | `true` | Interaction | EditAnywhere + BP | Through-wall interact protection source. Moonville's stock PreventInteractionThroughWall ray starts at the PAWN'S waist and clips the tier board under eye-level shelf pickups (E silently dead, 2026-08-05). true = pack ray disabled at BeginPlay; a CAMERA→item visibility gate protects the E press instead (see it = grab it; walls still block; InteractionArea overlap still required). false = stock waist ray. BeginPlay-applied — restart PIE. |
...
| `AZP_ProximityAggroRange` | `float` | `600.f` | Oozeling\|Detect | EditAnywhere + BP | UNAVOIDABLE aggro radius — inside it aggro fires when the player is seen OR navmesh-reachable (same space; walls/closed doors block both — 2026-08-05 through-wall fix); the lose-sight timer stays pinned inside it. |
...
| `AZP_ProximityAggroRange` | `float` | `600.f` | Scytheer|Detect | EditAnywhere + BP | UNAVOIDABLE aggro radius — inside it aggro fires when the player is seen OR navmesh-reachable (same space; walls/closed doors block both — 2026-08-05 through-wall fix); the lose-sight timer stays pinned inside it. |
...
| `AZP_FootstepSound` | `TObjectPtr<USoundBase>` | `null (lazy-filled at BeginPlay: /Game/Audio/Scytheer/SFX_Scytheer_Footsteps)` | Scytheer|Audio | EditAnywhere + BP | Footstep one-shot, fired per LIMB PLANT (bone-contact synced, Room SFX carry), played at the planting bone's world position; lazy placeholder fill if null. |
| `AZP_FootstepVolume` | `float` | `1.f` | Scytheer|Audio | EditAnywhere + BP | Footstep playback volume. 0 = silent feet. |
| `AZP_FootstepStride` | `float` | `80.f` | Scytheer|Audio | EditAnywhere + BP | FALLBACK ONLY (2026-08-05): distance (UU) of 2D travel per footstep, used only when no AZP_FootstepBones entry resolves on the mesh (mesh swap). Bone plants own the cadence otherwise. |
...
| `AZP_FootstepBones` | `TArray<FName>` | `Bip001_L_Hand_021, Bip001_R_Hand_027, Bip001_L_Toe0_034, Bip001_R_Toe0_040` | Scytheer|Audio | EditAnywhere + BP | ANIMATION-SYNCED footsteps (2026-08-05 v2): the 4 ground-contact bones of the quadruped gait (front hands + rear toes on the Biped rig). A step fires at each limb's TOUCHDOWN = the forward->backward sweep reversal of the bone in actor-relative space — pure animation signal, independent of body speed/play rate, all gaits incl. spline patrol. Offline-verified 4 touchdowns per walk loop. |
| `AZP_FootstepArmSpeed` | `float` | `50.f` | Scytheer|Audio | EditAnywhere + BP | Touchdown arming speed (uu/s): limb arms when its actor-relative forward sweep exceeds this; the next reversal fires the step. Walk-swing peaks ~130+; raise if phantom steps, lower if steps are missed. |
| `AZP_FootstepMinInterval` | `float` | `0.12f` | Scytheer|Audio | EditAnywhere + BP | Per-limb re-trigger lockout (s) so a jittery plant edge can't double-fire one contact. |
...
| `AZP_PipeWallImpactSound` | `TObjectPtr<USoundBase>` | `/Game/Audio/Weapons/SFX_PIPE_SURFACE_WALL_IMPACT (ConstructorHelpers)` | Kinemation|Melee | EditDefaultsOnly + BP | Pipe striking the CONCRETE/default surface family — and the fallback whenever a per-surface slot below is null. Family picked per hit by UZP_SFXStatics::ClassifySurface (phys-material name first, then material/mesh/actor name keywords; unmatched = concrete). |
| `AZP_PipeImpactMetal` | `TObjectPtr<USoundBase>` | `/Game/Audio/Weapons/SFX_PIPE_SURFACE_METAL_IMPACT (ConstructorHelpers)` | Kinemation|Melee | EditDefaultsOnly + BP | Pipe striking METAL (vents, lockers, machines, doors, railings, tanks). Ships as a duplicate of the wall impact — replace the asset's wave with the sourced sound (2026-08-05). |
| `AZP_PipeImpactWood` | `TObjectPtr<USoundBase>` | `/Game/Audio/Weapons/SFX_PIPE_SURFACE_WOOD_IMPACT (ConstructorHelpers)` | Kinemation|Melee | EditDefaultsOnly + BP | Pipe striking WOOD (crates, pallets, furniture, boards). Placeholder duplicate — replace the wave. |
| `AZP_PipeImpactGlass` | `TObjectPtr<USoundBase>` | `/Game/Audio/Weapons/SFX_PIPE_SURFACE_GLASS_IMPACT (ConstructorHelpers)` | Kinemation|Melee | EditDefaultsOnly + BP | Pipe striking GLASS/TILE (windows, lab tile, ceramics, screens). Placeholder duplicate — replace the wave. |
...
| `bAZP_Crawling` | `bool` | `false` | Shambler|Crawl | EditAnywhere + BP | THE STANCE SWITCH (dev 2026-08-05, vent-burst precursor). true = this shambler CRAWLS: crawl clips mask the standing BlendSpace via a Tick-maintained DefaultSlot loop, melee uses the crawl swipe, and standing-only behaviors are gated off (no grab, no alert-turn/scream pose — aggro barks + chases, no standing flinch clips; damage/mesh punch/stagger pauses still apply). Set per placed instance in Details. |
| `AZP_CrawlIdleAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_CrawlIdle)` | Shambler|Crawl | EditAnywhere + BP | Crawl idle loop (NAAT Crawl_Idle retargeted, retarget_shambler_crawl.py). |
| `AZP_CrawlWalkAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_CrawlWalk)` | Shambler|Crawl | EditAnywhere + BP | Slow crawl locomotion loop (NAAT Crawl_Slow). |
| `AZP_CrawlRunAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_CrawlRun)` | Shambler|Crawl | EditAnywhere + BP | Fast crawl loop for the sprint band (NAAT FastCrawl, 0.47s cycle). |
| `AZP_AttackCrawlAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_Attack_Crawl)` | Shambler|Crawl | EditAnywhere + BP | Crawl swipe (NAAT Attack_LH_Crawl, 2.77s). One of the crawler's random swing pool. |
| `AZP_AttackStandLHAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_Attack_StandLH)` | Shambler|Crawl | EditAnywhere + BP | Stand attack, left hand (NAAT Attack_LH_Stand, 1.47s) — the crawler REARS UP to strike. Empty the slot to remove it from the swing pool. |
| `AZP_AttackStandBHAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_Attack_StandBH)` | Shambler|Crawl | EditAnywhere + BP | Stand attack, both hands (NAAT Attack_BH_Stand, 1.93s) — rear-up strike. Empty to remove from the pool. |
| `AZP_StandAttackLHHitTime` | `float` | `0.85f` | Shambler|Crawl | EditAnywhere + BP | CLIP contact time (s) of the LH stand attack (same windup/strike rate remap as every swing). |
| `AZP_StandAttackLHWindupEndTime` | `float` | `0.6f` | Shambler|Crawl | EditAnywhere + BP | CLIP wind-up end (s) of the LH stand attack. |
| `AZP_StandAttackBHHitTime` | `float` | `1.1f` | Shambler|Crawl | EditAnywhere + BP | CLIP contact time (s) of the BH stand attack. |
| `AZP_StandAttackBHWindupEndTime` | `float` | `0.8f` | Shambler|Crawl | EditAnywhere + BP | CLIP wind-up end (s) of the BH stand attack. |
| `AZP_Preset` | `EShamblerPreset` | `Standing` | Shambler|Preset | EditAnywhere + BP | THE PRESET DROPDOWN (dev 2026-08-05): Standing Lurker (never fully pauses its wander) / Crawler (prone forever, motionless idle until aggro, crawls slow-near/fast-far, never stands) / Slumped (against a wall; rises through SlumpToStand on wake → normal standing shambler) / Playing Dead (corpse-still; ONLY proximity or damage wakes it → Crawler behavior). Legacy bAZP_Crawling=true maps to Crawler at BeginPlay. |
| `AZP_CrawlAlertSound` | `TObjectPtr<USoundBase>` | `null` | Shambler|Preset | EditAnywhere + BP | Crawler/PlayingDead aggro bark — separate slot for later replacement; null = standard alert vocal. |
| `AZP_CrawlFastDistance` | `float` | `800.f` | Shambler|Preset | EditAnywhere + BP | Target distance beyond which a chasing crawler uses the FAST crawl (clip + AZP_CrawlRunSpeed); inside it, the slow crawl. |
| `AZP_SlumpWakeRange` | `float` | `1500.f` | Shambler|Preset | EditAnywhere + BP | Slumped ROOM envelope (LOS-gated) inside which the presence/stare wake clocks run — being here alone does not wake it. Damage always wakes. |
| `AZP_SlumpWakePresenceTime` | `float` | `10.f` | Shambler|Preset | EditAnywhere + BP | Tier 1: continuous seconds in the room envelope (with LOS) before the slumped body wakes. |
| `AZP_SlumpWakeStareTime` | `float` | `3.f` | Shambler|Preset | EditAnywhere + BP | Tier 2: continuous seconds staring at the body (~18° view cone) before it wakes. |
| `AZP_SlumpProximityRange` | `float` | `500.f` | Shambler|Preset | EditAnywhere + BP | Tier 3: instant wake radius (500 = 5m). |
| `AZP_SlumpRiseRate` | `float` | `2.f` | Shambler|Preset | EditAnywhere + BP | Play rate on the slump rise (A_Shambler_SlumpToStand, 4.67s raw — 2.0 ≈ 2.3s). |
| `AZP_SlumpToStandAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_SlumpToStand)` | Shambler|Preset | EditAnywhere + BP | Slump pose/rise clip; frame 0 = the dormant against-the-wall pose (held ground-clamped). |
| `AZP_PlayDeadPoseAnim` | `TObjectPtr<UAnimSequence>` | `null (lazy default /Game/Enemies/Shambler/Anims/A_Shambler_ProneToStand)` | Shambler|Preset | EditAnywhere + BP | PlayingDead LIFELESS dormant pose — frame 0 held frozen (flat face-down, no torso lift; replaced AZP_PlayDeadIdleRate 2026-08-05). Crawl loops crossfade in on wake. |
| `AZP_ProximityAggroRange` | `float` | `600.f` | Shambler|Detect | EditAnywhere + BP | UNAVOIDABLE aggro radius (UU), every preset — inside it the hunt starts regardless of facing/noise, in the SAME SPACE: LOS, or a valid COMPLETE navmesh route ≤ max(2× straight, straight+300) (behind furniture passes). A wall/closed door = no route = blocked (2026-08-05 round-15: positive nav confirm replaced the lenient propagation veto). Also the PlayingDead wake radius. |
| `AZP_CrawlRotationRate` | `float` | `90.f` | Shambler|Crawl | EditAnywhere + BP | Crawler CMC yaw rate (deg/s) — low cap fixes the wild spinning from orient-to-movement jitter at crawl speeds. Standing rate restored on slump rise. |
| `AZP_CrawlDriveScale` | `float` | `1.f` | Shambler|Crawl | EditAnywhere + BP | BONE-DRIVEN propulsion (2026-08-05 round-22, replaces the sine pulse): the fastest backward limb sweep (hands/forearms/feet, actor space) BECOMES the walk speed each tick — surge timing and amount come from the playing animation. 1 = the animator's motion verbatim. |
| `AZP_CrawlDriveFloor` | `float` | `3.f` | Shambler|Crawl | EditAnywhere + BP | Speed floor (UU/s) between pulls — a trace of creep so paths never wedge. |
| `AZP_CrawlDriveCap` | `float` | `1.5f` | Shambler|Crawl | EditAnywhere + BP | Surge cap as a multiple of the state's base crawl speed. |
| ~~`AZP_CrawlPulseAmount` / `AZP_CrawlPulsesPerLoop` / `AZP_CrawlPulsePhase`~~ | `float` | — | Shambler|Crawl | REMOVED 2026-08-05 | Sine-pulse knob set replaced by the bone-driven AZP_CrawlDrive* trio above (surge timing/amount now read from the playing animation's limb sweeps, not a tuned sine). |
| `AZP_CrawlCapsuleHalfHeight` | `float` | `40.f` | Shambler|Crawl | EditAnywhere + BP | Crawler capsule half-height (UU), applied at BeginPlay — the standing ~88 capsule left the prone body riding at chest height ("it swims"). Mesh base rel-Z auto-compensates by the shrink so the skeleton stays planted (no float, no floor break) and AZP_CrawlMeshZOffset remains a 1:1 fine-tune. Clamped to [capsule radius, standing HH]. |
| `AZP_CrawlWanderSpeed` | `float` | `25.f` | Shambler|Crawl | EditAnywhere + BP | Crawler wander speed (standing: AZP_WanderSpeed). Was 60 — "waaaay too fast" (dev 2026-08-05). |
| `AZP_CrawlChaseSpeed` | `float` | `70.f` | Shambler|Crawl | EditAnywhere + BP | Crawler chase / fast-walk-phase speed (standing: AZP_ChaseSpeed). Was 180. |
| `AZP_CrawlRunSpeed` | `float` | `130.f` | Shambler|Crawl | EditAnywhere + BP | Crawler sprint-burst speed (standing: AZP_RunSpeed). Was 300. |
| `AZP_CrawlRunClipMinSpeed` | `float` | `100.f` | Shambler|Crawl | EditAnywhere + BP | Speed (UU/s) at/above which the FAST crawl clip replaces the slow crawl — the slow→fast handoff on its own dial, independent of the speed knobs. |
| `AZP_CrawlWalkAnimRefSpeed` | `float` | `70.f` | Shambler|Crawl | EditAnywhere + BP | Speed at which the crawl WALK clip's stride looks natural (loop rate = speed/this — hand-skate dial; crawl locomotion is slot-driven so it needs its own C++ ref speed). |
| `AZP_CrawlRunAnimRefSpeed` | `float` | `150.f` | Shambler|Crawl | EditAnywhere + BP | Speed at which the crawl RUN clip's stride looks natural. |
| `AZP_CrawlAttackHitTime` | `float` | `1.5f` | Shambler|Crawl | EditAnywhere + BP | CLIP time (s) of the crawl swipe's contact frame (AZP_AttackHitTime analog; same windup/strike rate remap). |
| `AZP_CrawlWindupEndTime` | `float` | `1.1f` | Shambler|Crawl | EditAnywhere + BP | CLIP time (s) where the crawl swipe's wind-up ends (AZP_WindupEndTime analog). |
| `AZP_CrawlBoneLift` | `float` | `6.f` | Shambler|Crawl | EditAnywhere + BP | THE prone ground-contact dial: height (UU) of the lowest contact bone above the capsule bottom (bones are joint centres — flesh hangs a limb-radius below). Shared by the runtime ground clamp AND the editor preview grounding, so placed == playing. Raise if buried, lower if floating (2026-08-05 round-18). |
| `AZP_CrawlIdleDebounce` | `float` | `0.5f` | Shambler|Crawl | EditAnywhere + BP | Sustained stillness (s) before the crawl loop yields to the idle pose. Brief nav-repath stalls keep the walk loop cycling; the instant flip restarted the loop at its right-arm pull every stall (2026-08-05 round-19). |
| `AZP_CrawlAttackRange` | `float` | `140.f` | Shambler|Crawl | EditAnywhere + BP | Crawl swing gate (UU) — prone claws are short/low; the standing 230 had crawlers swinging where they cannot connect (2026-08-05 round-24). Drives the chase hold, swing decision, and flurry re-check while prone. |
| `AZP_CrawlAttackHitRange` | `float` | `170.f` | Shambler|Crawl | EditAnywhere + BP | Crawl impact validation (UU) — back-stepping past this during the wind-up whiffs the hit (prone counterpart of AZP_AttackHitRange). |
| `AZP_CrawlMeshZOffset` | `float` | `-87.f` | Shambler|Crawl | EditAnywhere + BP | Mesh Z nudge (UU) for crawlers. MEASURED 2026-08-05: the crawl clips' contact plane sits ~87uu HIGHER in clip space than the standing toes (-10..-15 vs -100 below mesh origin) — the entire "it swims" float; -87 plants all three crawl loops within ~2uu of the floor. The Tick maintainer owns crawler mesh Z (slides to the STANDING offset during the reared-up window). |
| `AZP_CrawlHeadshotMinZ` | `float` | `0.f` | Shambler|Crawl | EditAnywhere + BP | Crawler headshot height (UU above body centre) — prone head sits near/below the capsule centre; the standing 55 makes headshots impossible. |
