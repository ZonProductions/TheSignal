...
| 2026-07-02 | On-body blood via STOCK decals on animated skeletal meshes (world-space AND bone-attached, both tried) | Stock UDecalComponent = projection box painting whatever geometry sits inside it — cannot stick to deforming skin. Proven 3 ways in PIE: world-space stains float in air; bone-attached boxes smear as surfaces swing through them; on death the box rides the spine down and paints the FLOOR. Enemy meshes keep ReceivesDecals=false. **VERIFIED SOLUTIONS (researched w/ citations, checkpoint 2026-07-02m):** (a) "Skinned Decal Component" Fab plugin (~$100, UV-independent, decals skin with bones/morphs — eyeball 5.8 in the listing dropdown before buying); (b) free: pre-skinned sphere-mask wounds (Looman/L4D2 pattern — ImpactPoint→bone ref-pose space→material params, SphereMask vs Pre-Skinned Local Position; no UV requirements, 3-5 wounds cap). UE 5.8 has NO native skinned decals (verified absence). UV-render-target painting requires non-overlapping UVs — risky on pack meshes. |
| 2026-07-02 | Python-created IK Retargeter (`add_default_ops` auto-map) crossing a COMBINED-neck-chain rig (UE4 `Head[neck_01->Head]`, CC_IK_Rig, MH_IK_Rig) into a split `Neck`+`Head` rig (UE5-family) | The combined source chain feeds BOTH target chains — neck+head rotation ~doubles ("top of head facing the floor", grab retargets). Cross such skeleton-family boundaries ONLY via pack-authored retargeters (RTG_UE4Manny_UE5Manny, MH_Retargeter); Python-created RTGs are safe only between rigs whose chains match 1:1 by name — or build a split-chain source rig first (IK_CCMH_Split pattern in retarget_grab_anims.py). Verify with a neck-total check (output vs source). See checkpoint 2026-07-02v (Content/checkpoints). |
| 2026-07-02 | DELETING dev-built BP event-graph logic on Claude's own judgment (stripped BP_Shambler's wander — BeginPlay → looping 3s timer → move to random reachable point — after probes showed it driving the pawn during grab-reaction windows) | The wander is DEV-BUILT, took multiple sessions, and is intended behavior — removing it was a critical regression, not a fix ("you can't just remove the wander"). The BP wander and the C++ behavior component coexist by design; the C++ masks it in normal play, and the correct fix for it winning during deliberately-paused windows (grab hold, reaction staggers, the loom) is a C++ GUARD: in TickComponent, while movement is frozen, `GetMoveStatus()==Moving → StopMovement()` — suppress moves only inside those windows, never touch the BP. (Diagnostic lesson stands: when an AI moves "for no reason," trace `GetMoveStatus()` against your own gating — [GrabSlideProbe] `moveStatus=Moving` while `staggered=1` = someone else is driving.) Graph restored 1:1 in behavior; AI MoveTo+Self recreated as GetController→SimpleMoveToLocation (MCP cannot create K2Node_Self / latent AIMoveTo). NEVER delete dev-built content without asking — deactivate around it in C++. |
| 2026-07-02 | `SetOverridePostProcessAnimBP(<graph-less C++ AnimInstance>)` on the Shambler mesh (grab arm-pose knobs) | A C++ AnimInstance with NO AnimGraph used as a post-process override replaces the pose path with REF POSE — and at the necromorph mesh's 0.01 base scale that rendered the Shambler INVISIBLE. (The same graph-less-C++ trick is fine for the NATIVE hooks — Marcus spine-bend works — but never as an override PP pose source.) FIX: run the native post-evaluate bone writes on the MAIN instance instead — reparent the AnimBP to the C++ class (ABP_Shambler → UZP_ShamblerGrabPoseAnimInstance, the UZP_GracePlayerAnimInstance pattern). |
| 2026-07-06 | Fixing invisible mirrored ISM/HISM instances by giving the component a negative-determinant transform (relative scale (1,1,-1)) so its draw call reverses backface culling | Consolidating single-sided meshes placed with negative scale (scale Z=-1, e.g. floor tiles flipped into a roof/ceiling) into an ISM/HISM makes them invisible from the intended side. A negative-scale COMPONENT transform does NOT flip instanced-mesh winding in UE5.8 — verified in PIE: the mirror relScale persisted (1,1,-1) yet the roof stayed invisible from below. FIX = TWO-SIDED material instance override on the component (MI with base_property_overrides two_sided=True; e.g. MI_Concrete_5_TwoSided), which disables culling entirely. Faithful single-sided alt = bake the Z-flip into a duplicated mesh (build_scale3d Z=-1) and instance with positive scale. See checkpoint 2026-07-06_roof_consolidation_hism. |
| 2026-07-12 | ProceduralMeshComponent as a walkable/visible surface (SM_Surface's original slab) in a Lumen level | Two independent PIE failures: (1) PMC generates NO runtime mesh distance fields, so Lumen can't represent it in the surface cache → renders PITCH BLACK under Lumen GI (documented engine limitation); (2) CMC landing never validates on the PMC trimesh — walking works, but LaunchCharacter (dodge) leaves the character in Falling forever → friction-free ice-slide until reaching a real static-mesh floor. FIX = real static-mesh instances (SM_Surface's SlabISM engine-cube tiles); the PMC survives only as an inert legacy root (NoCollision, no sections). See checkpoint 2026-07-12_sm_surface_pmc_retired_cube_slab. |
| 2026-08-04 | Dropping sg.ShadowQuality / sg.GlobalIlluminationQuality below Epic to buy GPU headroom in ResearchFacility | The horror lighting is CALIBRATED against Epic-tier Lumen — Medium GI rendered the environment "beyond broken and unrecognizable" (dev, immediate revert). GPU did drop 20-26ms -> 8.5ms, proving the cost is tier-bound, but the look constraint kills the lever permanently. Buy GPU headroom via true PIE resolution scaling or per-feature cvars that don't shift the lighting calibration. Editor tier lives in Saved/Config/WindowsEditor/EditorSettings.ini (now explicit Epic). |
| 2026-08-04 | `OnlyTickPoseWhenRendered` on START-HIDDEN skeletal meshes (player melee/ranged view models, grab-only heads) | First-show deadlock: never ticked -> stale bounds -> frustum-culled -> never renders ("arm didn't fully render" on pipe equip). Only safe on meshes that begin visible/rendered (enemy world meshes keep it). Reverted to AlwaysTickPoseAndRefreshBones on all 9 player meshes. |
| 2026-07-12 | Python slate-post-tick callback calling `get_all_level_actors()` (or any actor iteration) without a world guard (viscut_watch.py) | Asset opens / map loads tick Slate from their progress dialog; the callback fires mid-transition while the editor world context is NULL → `TActorIterator` `check(CurrentWorld)` (EngineUtils.h:185) = hard editor CRASH. Python try/except CANNOT catch a C++ assert. Every slate-tick Python callback must verify `UnrealEditorSubsystem.get_editor_world()` returns a VALID world BEFORE any actor/world API. See checkpoint 2026-07-12_viscut_watch_null_world_crash. |
| 2026-08-04 | Trusting `duplicate_and_retarget` to emit ABSOLUTE clips + verifying retarget output by rotation-delta only (retarget_shambler_turns2.py) | `IKRetargetBatchOperation.duplicate_and_retarget` COPIES the SOURCE asset's additive settings + ForceRootLock onto the output. The SLS `Idle/Turn_*` sources are additive (AAT_LOCAL_SPACE_BASE vs own frame 0, force_root_lock=True), so all four `A_Shambler_Turn_*` clips evaluated as delta-vs-frame-0 AND root-locked to the necromorph ref pose (hips IS the root; ref scale 0.0001) — skeleton collapsed to a point = mesh INVISIBLE during alert turns + blank content-browser previews (the 07-14 FloatProbe "+101 toe float, clip authored ~1m up" was this same symptom, misfiled). Rotation-only probes CANNOT catch it: `get_bone_pose_for_time` bypasses additive processing and masks the root track (read full-size values on the broken clips). FIX = clear `additive_anim_type`/`ref_pose_type`/`force_root_lock` on every retarget output (now baked into retarget_shambler_turns2.py) and verify SIZE via `AnimPoseExtensions.get_anim_pose_at_time` (hips scale == 1, spine \|T\| ≈ 928), never rotation deltas alone. See checkpoint 2026-08-04_shambler_turn_additive_flags_invisible_fix. |
