...
reenable = ['AC_FirstPersonCamera_C', 'AC_TacticalShooterAnimation_C', 'AC_RecoilAnimation_C', 'AC_IKMotionPlayer_C']
for c in pawn.get_components_by_class(unreal.ActorComponent):
    cname = c.get_class().get_name()
    if cname in reenable:
        c.set_component_tick_enabled(True)
        unreal.log(f'[BISECT] Re-enabled: {cname}')

unreal.log('[BISECT] Camera Z=0, all Kinemation comps ticking')
