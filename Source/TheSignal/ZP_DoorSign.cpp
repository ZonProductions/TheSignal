...
AZP_DoorSign::AZP_DoorSign()
{
	...
	{
		AZP_SignBackgroundMesh = CubeMesh.Object;
		Background->SetStaticMesh(AZP_SignBackgroundMesh);
	}
	...
	{
		AZP_SignBackgroundMaterial = SignMat.Object;
		Background->SetMaterial(0, AZP_SignBackgroundMaterial);
	}

	// Flat panel facing +X: ~0.5cm thick (X), ~30cm wide (Y), ~10cm tall (Z)
	Background->SetWorldScale3D(AZP_SignPanelScale);
	...
	SignText->SetText(AZP_DefaultSignText);
	SignText->SetWorldSize(AZP_SignTextSize);
...
void AZP_DoorSign::TryAttachToSurface()
{
	...
	const FVector End = Start - GetActorForwardVector() * AZP_AttachTraceDistance;
