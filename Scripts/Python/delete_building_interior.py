...
for a in eas.get_all_level_actors():
    ...
    if lbl.startswith(ENV): continue
    cs=a.get_components_by_class(unreal.StaticMeshComponent)
    ...
    if cls in CONTENT_CLASS:
        ...
        if "door" in mnl: keep=False
        elif mn in KEEP_MESH: keep=True
        elif mn=="SM_FrameTall": keep=True
        elif mn=="SM_Cube":
            if e.z<80: keep=True
            elif has_glass(c): keep=True
            elif any(k in ll for k in WALL_KEEP_LABEL): keep=True
            elif interior(o,e): keep=False
            else: keep=True
        elif mnl in ("cylinder","sm_cylinder"): keep=(e.z>=150)
        else: keep=False
    else:
        keep=True
    if not keep: remove.append(a)
...
for a in remove:
    ...
    man.append({"label":a.get_actor_label(),"class":a.get_class().get_name(),
                "mesh":sm.get_name() if sm else None,"x":round(l.x,1),"y":round(l.y,1),"z":round(l.z,1)})
with open(r"C:/Users/Ommei/workspace/TheSignal/Scripts/Python/_removed_building_empty_manifest.json","w") as f:
    json.dump(man,f,indent=1)
print("interior actors to delete:", len(remove))
...
# clean up now-empty GroupActors
emptied=0
for a in list(eas.get_all_level_actors()):
    if a.get_class().get_name()=="GroupActor":
        try:
            if hasattr(a,"get_grouped_actors"):
                if len(a.get_grouped_actors())==0:
                    eas.destroy_actor(a); emptied+=1
        except Exception: pass
print("empty groups removed:",emptied)
