...
| `AZP_AttackRange` | `float` | `200.f` | Scytheer|Attack | EditAnywhere + BP | Distance at which a chasing Scytheer starts a swipe; also seeds the MoveToActor acceptance radius (AttackRange - 60). |
| `AZP_AttackCooldown` | `float` | `0.7f` | Scytheer|Attack | EditAnywhere + BP | Minimum seconds between attack swings. |
| `AZP_AttackDamage` | `float` | `20.f` | Scytheer|Attack | EditAnywhere + BP | Damage applied to the player at the midpoint of an attack clip. |
...
| `AZP_AlertHoldTime` | `float` | `2.0f` | Scytheer|Alert | EditAnywhere + BP | Seconds the Scytheer holds the idle alert pose (facing the player, alert SFX) before starting the chase. |
...
| `AZP_AttackSound` | `TObjectPtr<USoundBase>` | `null (lazy-filled at BeginPlay: /Game/Audio/Scytheer/SFX_Scytheer_Attack)` | Scytheer|Audio | EditAnywhere + BP | Sound played on starting an attack swing; lazy placeholder fill if null. |
...
| `AZP_FlinchCooldown` | `float` | `0.5f` | Shambler|Anim | EditAnywhere + BP | Min seconds between cosmetic flinch-clip plays / swing hitches (mesh punch is never gated). |
